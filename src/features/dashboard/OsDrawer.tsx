...
export const OsDrawer = ({ osId, onClose }: OsDrawerProps) => {
  ...
  const handleStatusChange = async () => {
    ...
    try {
      await orderApi.updateOrderStatus(osData.id, newStatus);
      // Wait for React Query to invalidate and re-fetch through top-level components or trigger it directly
  ...
  useEffect(() => {
    ...
    const fetchOsDetails = async () => {
      ...
      try {
        ...
        const found = orders.find((os: any) => os.id === osId);
      ...
      <div className={`fixed top-0 right-0 h-full w-[90%] md:w-[50%] lg:w-[40%] bg-zinc-950 border-l border-zinc-800 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col translate-x-0`}>
        ...
        <div className="flex items-center justify-between p-6 border-b border-zinc-800 bg-zinc-900/50">
          <div className="flex items-center gap-4">
            <h2 className="text-xl font-bold text-white m-0">OS #{osData?.numeroOs || '...'}</h2>
            {osData && (
              <>
                ...
                {(osData.valorMaoObra || osData.valorPecas) ? (
                  <span className="text-lg font-bold text-green-400 ml-4">
                    {formatBRL((osData.valorMaoObra || 0) + (osData.valorPecas || 0))}
                  </span>
                ) : null}
              </>
            )}
            ...
            <>
                ...
                {osData.garantiaDias && (
                  <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-blue-400/80 text-xs">{osData.garantiaDias} dias de garantia para os serviços executados.</p>
                    </div>
                  </div>
                )}
              </section>
              ...
              <section>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {osData.fotosCheckin && osData.fotosCheckin.length > 0 ? (
                    osData.fotosCheckin.map((fotoStr, i) => (
                      <div 
                        key={i} 
              ...
              <section>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {osData.checklistItens?.map((item: any) => (
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                  {MOCK_STATUS_HISTORY.filter(h => h.osId === osData.id).map((hist, i, arr) => (
                    <div key={hist.id} style={{ display: 'flex', gap: 12, position: 'relative' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                        <div style={{
                          width: 8, height: 8, borderRadius: '50%', marginTop: 4, flexShrink: 0,
                          background: STATUS_CONFIG[hist.statusNovo as OSStatus]?.cor || 'rgba(255,255,255,0.3)',
                        }} />
                        {i < arr.length - 1 && (
                      ...
                      <div style={{ paddingBottom: 16, flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
                          <span style={{ fontSize: 12, fontWeight: 600, color: STATUS_CONFIG[hist.statusNovo as OSStatus]?.cor || 'rgba(255,255,255,0.6)' }}>
                            {STATUS_CONFIG[hist.statusNovo as OSStatus]?.label}
                          </span>
                          <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)' }}>
                            {new Date(hist.criadoEm).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) }
                          </span>
                        </div>
                        <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)' }}>por {hist.changedByNome}</span>
                        {hist.notaPublica && (
                          <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.50)', marginTop: 4, lineHeight: 1.5 }}>{hist.notaPublica}</p>
                        )}
                      </div>
                    </div>
                  ))}
                  {MOCK_STATUS_HISTORY.filter(h => h.osId === osData.id).length === 0 && (
